...
export function PujoMusicPlayer({ isOpen, onClose }: PujoMusicPlayerProps) {
  ...
  // Send volume to the YouTube player whenever volume/mute changes
  useEffect(() => {
  ...
  // Lock background website scrolling only when full modal is open
  useEffect(() => {
  ...
  // Update track duration when active track changes
  useEffect(() => {
    setCurrentTime(0);
    setSeekTime(null);
    setIsAudioStarted(false);
  ...
  return (
    <>
      ...
      {/* 2. MINIMIZED FLOATING PLAYER UI */}
      {isOpen && isMinimized && (
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#f8edd8]/70">
                    {bengali ? `${PUJO_MUSIC_TRACKS.filter(t => t.section === "hits").length}টি সেরা পুজো গান ও আগমনী সুর` : `${PUJO_MUSIC_TRACKS.filter(t => t.section === "hits").length} Festive Pujor Gaan & Mahalaya`}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center rounded-3xl border border-[#f5c85b]/30 bg-gradient-to-b from-[#2e1214] to-[#1a0b0c] p-6 text-center shadow-2xl">
                ...
                <div className="mt-4 flex items-center justify-center gap-4 relative">
                  ...
                  <a
                    ...
                    className="flex items-center gap-1.5 rounded-2xl border border-white/20 bg-white/10 px-3 py-2 text-xs font-bold text-[#f5c85b] transition hover:bg-white/20"
                ...
                >
                  {currentSectionTracks.map((t, idx) => {
                    ...
                    const formatTrackDuration = (secs?: number) => {
                      ...
                      if (hours > 0) {
                        return `${hours}:${mins < 10 ? "0" : ""}${mins}:${remaining < 10 ? "0" : ""}${remaining}`;
                      }
