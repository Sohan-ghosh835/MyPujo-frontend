      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {visibleFestival.map((image, index) => (
          <article key={`festival-context-${index}`} className="overflow-hidden rounded-2xl bg-[#fff9eb] text-[#4a2520] shadow-[0_10px_22px_rgba(0,0,0,.18)]">
            <img src={image.url} alt={image.alt} loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={(e) => { (e.currentTarget as HTMLImageElement).src = "/durga-puja-hero.png"; }} className="h-48 w-full object-cover" />
            <div className="p-3">
