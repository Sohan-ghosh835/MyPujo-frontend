...
export default function PandalDetail() {
  ...
  return <AppShell><div className="container py-9 sm:py-12">
    ...
    <div className="mt-6 overflow-hidden rounded-[1.7rem] bg-[#3a1c1d] text-[#fff8e8] shadow-[0_22px_50px_rgba(53,27,19,.2)] lg:grid lg:grid-cols-[.9fr_1.1fr]">
      {pandal.image ? <div className="relative min-h-[310px] bg-[#2e1718]"><img src={pandal.image.url} alt={pandal.image.alt} referrerPolicy="no-referrer" onError={(e) => { (e.currentTarget as HTMLImageElement).src = "/durga-puja-hero.png"; }} className="absolute inset-0 z-0 h-full w-full object-cover object-center" /><div className="absolute inset-0 z-10 bg-gradient-to-t from-[#2f1819]/70 via-[#2f1819]/15 to-transparent" /><span className="absolute bottom-5 left-5 z-20 rounded-full bg-black/30 px-3 py-1 text-[10px] font-bold uppercase tracking-[.15em]">{bengali ? "ঐতিহাসিক ছবি" : "Historical photo"} · {pandal.image.capturedYear}</span></div> : <div className="flex min-h-[310px] flex-col justify-end bg-[#f0e4d0] p-7 text-[#4a2520]"><MapPin className="text-[#9a2223]" size={24} /><p className="eyebrow mt-5">{bengali ? "ঠিকানা রেকর্ড" : "Address record"}</p><h2 className="font-display mt-2 text-3xl font-bold">{bengali ? "যাচাই করা জনসাধারণের ছবি যুক্ত নেই।" : "No verified public photograph is attached."}</h2><p className="mt-3 max-w-sm text-sm leading-relaxed text-[#73584b]">{bengali ? "নিচের সরবরাহ করা ঠিকানা, ম্যাপ প্রিভিউ ও উৎস লিঙ্ক ব্যবহার করুন। PujoParikroma সম্পর্কহীন বা লাইসেন্সবিহীন ছবি বসায় না।" : "Use the supplied address, map preview, and source links below. PujoParikroma does not substitute an unrelated or unlicensed image."}</p></div>}
